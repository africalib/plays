...
let app = new Vue({
    ...
    methods: {
        ...
        saveReplay: function () {
            ...
            if (replays) {
                ...
                while (replays.length > 10) {
                    // 인덱스 0은 preview 이면 인덱스 1 삭제
                    if (replays[0].name === 'preview') {
                        replays.splice(1, 1);
                    }
                    else {
                        replays.splice(0, 1);
        ...
        goHome: function () {
            if (!this.status.replay && this.status.started
                && this.flows.length > this.base.minFlowLength
                && !this.hasReplay() && confirm('해당 경기를 리플레이로 저장하시겠습니까?')) {
                this.saveReplay();
            }
            ...
            for (let i in this.areas) {
                ...
                if (unit.name) {
                    if (unit.player === player) {
                        ...
                        let restoreHp = 0;
                        let inShelterOrArea = unit.name === 'king' || this.isInShelterOrArea(i);
